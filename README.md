...
标准化视觉感知插件。**双流异步架构**：轻量行为检测（毫秒级）+ 重量语义分析 + A→B 级联管线。
...
Pipeline 级联   A 触发 B: 大幅动作 → 语义分析 → 记忆回调 → 跨模态锚定
```
...
| `motion_burst` | 运动从静到动（原始 motion>0.15） | B 行为-对象分析（"动作:xx 对象:xx"）+ 跨模态锚定 |
...
- **运动熵门控**：剧烈运动不硬分析（避免幻觉），事后补全——模拟人类"先感知动态，再聚焦物体"
...
| `anchor_cooldown` | 1800s | 跨模态锚定冷却（防打扰） |
...
# 命中示例: 画面出现"登山鞋" → 记忆里有"买了新登山鞋" → 锚定 → 上层主动话:"你拿的登山鞋，是上次买的那个吗？"
```
...
**Q: storage 需要什么接口？**
A: 只需 `remember(text, room="chat", metadata=None, dedupe=True) -> str`；要跨模态锚定再加 `recall_with_scores(query, n_results=5, room=None) -> list[dict]`。memory-plugin 完全满足。
